...
public class SpiderUtils {
    ...
    public static String getItemName(Document curDoc) {
        String itemName = curDoc.select("div.sku-name").text();
        return itemName;
    }
    ...
    public static List<String> getProductPara(Document curDoc) {
        ...
        Elements productParaEles = curDoc.select("#detail > div.tab-con > div:nth-child(1) > div.p-parameter > ul.parameter2.p-parameter-list > li");
    ...
    public static List<String> getDescImgsURL(Document curDoc) {
        ...
        Pattern pDesc = Pattern.compile("desc:\\s*'(.*?)'");
        ...
        if (mDesc.find()) {
            ...
            try {
                ...
                //过滤处jpg大图片
                Pattern pImg = Pattern.compile("data-lazyload=\"(.*?)\"");
                Matcher mImg = pImg.matcher(descText);
                int imgIndex = 0;
                while (mImg.find()) {
                    if (mImg.group(1).endsWith("jpg")) {
                        imgIndex += 1;
                        descImgsUrlList.add("https:" + mImg.group(1));
                    }
    ...
    public static Set<String> getAllPages(String url, int pagesNum, String catUrl) {
        ...
        try {
            Document descDoc = Jsoup.connect("https://module-jshop.jd.com/module/getModuleHtml.html?appId=813393&orderBy=99&pageNo=1&direction=1&categoryId=0&pageSize=24&pagePrototypeId=8&pageInstanceId=84209352&moduleInstanceId=84209355&prototypeId=55555&templateId=905542&layoutInstanceId=84209355&origin=0&shopId=725756&venderId=730730&callback=jshop_module_render_callback&_=1515563921165")
                    .userAgent("Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.221 Safari/537.36 SE 2.X MetaSr 1.0")
                    .ignoreContentType(true)
                    .timeout(3000)
                    .get();
...
//                        .get();
            ...
            Elements itemEles = descDoc.select("#J_GoodsList > ul > li > div > div.jPic > a");
